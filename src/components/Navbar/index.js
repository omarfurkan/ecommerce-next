...
function NavItems({ isModalView = false }) {
    ...
    return (
        <div id="nav-items" className={`item-center justify-between w-full md:flex md:w-auto ${isModalView ? '' : 'hidden'}`}>
            <ul className={`flex flex-col p-4 md:p-0 mt-4 font-medium border rounded-lg md:flex-row md:space-x-8 md:mt-0 md:border-0 bg-white ${isModalView ? 'border-none' : 'border border-gray-100'}`}>
                {
                    isAdminView
                        ? adminNavOptions.map((item) =>
                        (<li key={item.id} className="cursor-pointer block py-2 pl-3 pr-4 text-gray-900 rounded md:p-0">{item.label}
                        </li>))
                        : navOptions.map((item) =>
                        (<li key={item.id} className="cursor-pointer block py-2 pl-3 pr-4 text-gray-900 rounded md:p-0">{item.label}
                        </li>))
            ...
            <nav className='bg-white w-full z-20 top-0 left-0 border-b border-gray-200'>
                <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
                    ...
                    <div className='flex md:order-2 gap-2'>
                        ...
                        {
                            user?.role === 'admin' ? isAdminView ? <button className="mt-1.5 inline-block bg-black px-5 py-3 text-xs font-medium upprcase tracking-wide text-white">Client View</button> : <button>Admin View</button> : null
                        }
